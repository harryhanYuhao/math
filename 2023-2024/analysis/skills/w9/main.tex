...
%Title page
\title[An Interesting Lebegues Intergrable Function] %optional
...
%---------------------------------------------------------
\begin{frame}
\frametitle{Lebegues Intergral is a Generalization of Riemann Integral}

\begin{block}{All Reimain Integrable Functions are Lebegues Integrable}
\end{block}

\begin{alertblock}{\textbf{Not} All Lebegues Integrable Functions are Reimann Integrable}
	...
	We have:
	$$(L) \int \mathcal{D} = 0$$
	As Lebegues integral over countable set is 0. 
\end{alertblock}
...
\begin{block}{Example}
	...
	\begin{figure}
		\begin{tikzpicture}[scale = 0.6,domain=-0.2:2]
			...
			\draw[->] (0,-2.2) -- (0,2.2) node[above] {$y$};
			\draw[color=orange, line width = 0.25mm, domain=0.11:2] plot (\x,{1/((\x)^(1/2))}) node[right] {$f(x) = \frac{1}{x^{1/2}}$};
			\draw[color=blue, line width = 0.25mm, domain=0.61:3] plot (\x,{1/((\x)^(2))}) node[right] {$f(x) = \frac{1}{x^{2}}$};
		\end{tikzpicture}
	\end{figure}
\end{block}
