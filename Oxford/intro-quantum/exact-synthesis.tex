\section{Exact Synthesis of Clifford+T Circuits}

In this section we present results from Kliuchnikov et al \cite{exact-clifford+t} an algorithm for the exact synthesis of single qubit unitaries over the Clifford+T gate set, when it is possible.
...
\begin{theorem}\label{unitary-iff-clifford+t}
	...
	In such case, moreover, it can be written in the following form
...
Least denominator exponent is a measure of how far a unitary matrix is from being implementable by Clifford gates alone, as Clifford gates have entries in $\Z[i]$ and thus have least denominator exponent $0$.
...
As the column of a unitary matrix $U \in \mathcal{CT}(2)$ is a unit-vector, we can reduce $\text{ldt-tl}(U)$ it to small enough number, after which we can find a Clifford+T circuit that implements it by brute-force search.

We list the exact steps of in algorithm \ref{exact-synthesis-algorithm} for clarity.
...
So the run time of the algorithm is linear in number of the gates in the output circuit.
