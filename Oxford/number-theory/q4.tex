...
In question 1, we have established that if $p \nmid h(\Qd)$, the coprime solutions $(x, y)$ for the equation 
...
For $p \geq 5$, let us consider the following two equations 
\begin{align}
	3^p &= 5^2 + k_1 \label{contra1}\\
	3^p &= 7^2 + k_2 \label{contra2}
\end{align}
where $k_1 = 3^p - 5^2$ and $k_2 = 3^p - 7^2$.
In both case, we have constructed coprime solutions $(x, y) = (5, 3)$ and $(x, y) = (7, 3)$ for respective $k_1, k_2$ of \autoref{eq:4main}.
...
Write $k_1 = r_1^2 d_1$ where $d_1 \in \Z$ is square-free.
Since $ k_1 \equiv 2 \mod 4$, we have $2 | k_1$ while $4 \nmid k_1$, so $2 |d_1$. 
If $\frac{k_1}{2}$ is not a perfect square, there is another prime $q \neq 2$ that divides $d_1$, making $d_1 > 3$.
In such case the solution $(x = 5, y = 3)$ for \eqref{contra1} satisfies $y < d_1$, leaving $p | h(\Q(\sqrt{-d_1}))$.
The same arguments apply for $k_2$ and \eqref{contra2} as well.

Note that $\frac{k_1}{2} + 12 = \frac{k_2}{2}$ and we have 
\begin{align}
	7^2 - 6^2 &= 13 \\ 
	(a+1)^2 - a^2 &> 13 \text{ for any } a > 6
\end{align}
If $\frac{k_1}{2} > 36 = 6^2$, it is impossible that both of $\frac{k_1}{2}$ and $\frac{k_2}{2}$ are perfect squares, and our $p$ divides at least one of $h(\Q(\sqrt{-d_1}))$, $h(\Q(\sqrt{-d_2}))$

It is clear that $k_1 > 36$ for all $p \geq 5$. 
So our arguments above work for all $p \geq 5$.
...
In either case, we have $k_i < 3 ^p $. 
As $d| k_i$, it implies that 
$$
d < 3^p.
$$
Taking the logarithm base 3 on both sides gives us
$$
p > \frac{\log(d)}{\log(3)} 
$$
...
Since the above arguments only work for $p \geq 5$, we still need to check the case $p = 3$.
This leaves no problem as $h(\Q(\sqrt{-26})) = 6$ is divisible by 3, and $6 = h(\Q(\sqrt{-26})) > \frac{\log(26)}{\log(3)} \approx 2.97$.
See \autoref{tb:solutions} for more examples of class group constructions.
