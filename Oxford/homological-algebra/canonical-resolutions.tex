...
\begin{definition}[Contractible]
	A simplicial object $X$ is \emph{contractible} if there exists maps $f_{n}: X_n \to X_{n+1}$ for each $n\geq 0$ such that
	\begin{align}
...
\begin{proof}
	Recall that $\perp = FU, \top = UF, \eta: \id \Rightarrow \top, \epsilon: \perp \Rightarrow \id$ are the comonad and monad structures associated to the adjunction $(F, U)$.

	Unwrapping the notation, the simplicial object $U(\perp_\bullet A)$ has terms
	\[	U(\perp^n A) = U(FU)^n A = \top^n U(A) \]
	and face and degenerate maps given by
	\begin{align*}
		\d_i &= U \perp^i \epsilon \perp^{n - i} = \top^i U(\epsilon_{\perp^{n - i} A}) \\
		\s_j &= U \perp^j \eta \perp^{n - j} = \top^j \eta_{U(\perp^{n - j} A)}
	\end{align*}
	
	Let us define $f_n: U(\perp^n A) \to U(\perp^{n+1} A)$ as
	\[
		f_n = \top^{n+1} \eta U
	\]
	Let us verify that $f_n$ satisfies \eqref{contractible1} and \eqref{contractible2}.

	By property of adjoint we have the following commutative diagram:
	$$
	\begin{tikzcd}
			U(A) \arrow{r}{\eta_{U(A)}} \arrow{dr}{\text{id}_{U(A)}} & UFU(A) \arrow{d}{U(\epsilon_{A})} \\
			 & U(A)
	\end{tikzcd}
		$$
		Applying the functor $\top^{n+1}$ to the diagram gives us
		$$
		\top^{n+1} U (\epsilon_A)\circ \top^{n+1} \eta_{U(A)} = \id
		$$
		This is exactly 
		\[
			\d_{n+1} f_n = \id 
		...
		The verification of \eqref{contractible2} is similar, as we have 
		$$
		\d_i f_n = (\top^n \eta U)(U \perp^i \epsilon \perp^{n - i} ) = f_{n-1}\d_i
		$$
\end{proof}
\begin{remark}
	Weibel \cite{weibel} defines $f_n = \eta U\perp^n$ in proposition 8.6.10.
	I could not verify the contractible condition with this definition.
\end{remark}
...
\section{Relative Homology and Hochschild Homology}
...
Therefore $(F, U)$ constitutes a comonad $\perp = FU$ on $\MR$, and thus we can construct a simplicial object $\perp_\bullet M$ for any $M \in \MR$.
