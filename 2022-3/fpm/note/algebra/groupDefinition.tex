...
\begin{enumerate}
	...
	\item Inverse: $\forall a\in \mathbb{S}, \exists a^{-1}\in \mathbb{S}$ such that $a\odot a^{-1} = a^{-1}\odot a = e$.
\end{enumerate}
...
\begin{remark}
	...
	It is important to note that for the definition of identity can not be simplified to $\exists e\in \mathbb{S}$ such that $a\odot e = a$ for all $a\in \mathbb{S}$. 
	Nothing has prevented us from arbitrating that for a certain set $\bb{S}$ and $a, b \in \bb{S}$, it could be that $a \odot b = a$ while $b \odot a\neq a$.`
\end{remark}
