...
\begin{definition}[Classification of Diferential Equation]
	\begin{enumerate}
...
More significantly, it means that $ \frac{f_1}{f_2} = g(t),$ where g(t) is not a constant function of $t$ in their domain.
\end{definition}

\begin{theorem}[Nature of Solutions to secound order linear ODE]
	A second order linear ODE's solution would be two and only twosets of linearly independent functions.
\end{theorem}
\subsubsection{General Solutions to Second Order Homogeneous ODE}
...
\begin{equation}\label{eq:2NHOMOODE}
	ay'' + by' + cy = g(t)
\end{equation}
Suppose the homogeneous equation ($ayy'' + by' + cy = 0$) has the general solution:
\[
